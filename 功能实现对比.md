...
| 菜品营养成分查询 | ✅ 已实现 | `routers/food.py` - `analyze_food` | AI分析返回营养数据 |
...
| 基于用户健康目标的菜品推荐 | 计划书2.2.1 | 中 |
| 基于历史偏好的推荐 | 计划书2.2.1 | 低 |
...
| 环境变化监测与Plan B生成 | 计划书2.2.5(三) | 中 |
| ~~运动记录新增/查询（实际执行记录）~~ | ~~计划书2.2.5(四)~~ | ~~中~~ | ✅ 已完成 (Phase 25)
| 实际消耗热量计算与达成率统计 | 计划书2.2.5(四) | 中 |
| 饮食-运动数据联动闭环 | 计划书2.2.2 | 中 |
| 离线运动包生成与下载 | 计划书2.2.1 | 低 |
| 运动偏好意图理解 | 计划书2.2.1 | 低 |
...
| 运动频率统计 | 计划书2.2.6 | 低 |
| 健康目标达成率计算 | 计划书2.2.6 | 低 |

### 四、AI能力中台服务

| 功能 | 需求来源 | 优先级 |
|------|---------|--------|
| Milvus向量数据库集成 | 计划书2.1.2 | 高 |
| BGE-M3嵌入模型集成 | 计划书2.2.1 | 高 |
| 营养知识库构建与检索（RAG） | 计划书2.2.1 | 高 |
| 菜谱知识图谱构建与检索 | 计划书2.2.4 | 高 |
| 运动消耗库检索 | 计划书2.2.5 | 中 |
