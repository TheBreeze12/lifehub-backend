...
| 一键遗忘（清除历史数据，保留账号） | ✅ 已实现 | `routers/user.py` - `delete_user_data`, `services/user_service.py` | Phase 55新增，级联删除所有历史数据并重置偏好，保留用户账号 |
...
| ~~AI调用记录/日志查看~~ | ~~计划书2.2.1~~ | ~~中~~ | ✅ 已完成 (Phase 56) `app/db_models/ai_call_log.py` + `app/services/ai_log_service.py` + `GET /api/user/ai-logs` + `GET /api/user/ai-logs/stats` 自动记录所有6种AI调用(food_analysis/menu_recognition/trip_generation/exercise_intent/meal_comparison/allergen_check)并提供查询和统计接口，前端AiCallLogPage展示调用时间、类型、数据摘要 |
| ~~Few-shot Prompt模板管理 + 意图理解增强~~ | ~~计划书2.2.1~~ | ~~低~~ | ✅ 已完成 (Phase 57) `app/services/prompt_template_service.py` 集中管理6种AI调用Prompt模板，所有AI调用方法均接入模板服务，支持运动偏好意图槽位提取(运动类型/时长/强度)，支持Few-shot示例、动态变量替换、模板版本管理、JSON持久化 |
...
| 请求限流与熔断 | 计划书2.2.1 | 中 |
| ~~数据脱敏处理~~ | ~~计划书2.1.4~~ | ~~中~~ | ✅ 已完成 (Phase 54) `app/middleware/data_masking.py` 日志脱敏+坐标模糊化+响应敏感字段脱敏；已修复content-length不匹配导致客户端"unexpected end of stream"错误 |
